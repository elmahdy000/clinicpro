...
3.  **دقة التوقيت والمناطق الزمنية:**
    *   تسجل قاعدة البيانات كل الأوقات بصيغة `UTC` العالمية وتخزين التواريخ كـ ISO Strings.
    *   **هام:** يجب استخدام موديول المساعد المركزي `api/src/common/helpers/timezone.helper.ts` في جميع موديولات النظام (`AppointmentsService`, `DoctorsService`, `DashboardService`) لإجراء أي عمليات تحويل أو حساب حدود الأيام (Day Bounds) أو تفكيك التواريخ للتطابق مع المنطقة الزمنية للعيادة (المخزنة في `ClinicSettings` مثل `Africa/Cairo`)، وذلك لمنع أي انحرافات زمنية ناتجة عن التوقيت المحلي للخادم (Server Local Time).
    *   الوظائف المتاحة في المساعد:
        *   `parseLocalToUtc(dateStr, timezone)`: تحويل التاريخ والوقت المحلي للعيادة إلى UTC.
        *   `getLocalDayBoundsInUtc(dateStr, timezone)`: حساب بداية ونهاية اليوم المحدد بالتوقيت المحلي للعيادة وإرجاعهما كـ UTC.
        *   `getLocalDateStr(date, timezone)`: استخراج تاريخ اليوم المحلي للعيادة كـ `YYYY-MM-DD` من كائن تاريخ UTC.
