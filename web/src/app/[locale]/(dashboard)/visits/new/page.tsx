...
const COMMON_MEDICINES = [
  { name: 'بانادول', dosage: '500mg', frequency: '3 مرات يومياً', duration: '3' },
  { name: 'بروفين', dosage: '400mg', frequency: 'مرتين يومياً', duration: '5' },
  { name: 'أموكسيسيلين', dosage: '500mg', frequency: '3 مرات يومياً', duration: '7' },
  { name: 'كونجستال', dosage: 'كيس', frequency: '3 مرات يومياً', duration: '3' },
  { name: 'سولبادين', dosage: 'قرص', frequency: 'عند اللزوم', duration: '' },
  { name: 'سيبروفلوكساسين', dosage: '500mg', frequency: 'مرتين يومياً', duration: '7' },
  { name: 'أومبيرازول', dosage: '20mg', frequency: 'مرة يومياً', duration: '14' },
  { name: 'فينتولين', dosage: 'بخة', frequency: 'عند اللزوم', duration: '' },
  { name: 'سيتريزين', dosage: '10mg', frequency: 'مرة يومياً', duration: '7' },
  { name: 'باراسيتامول', dosage: '500mg', frequency: '3 مرات يومياً', duration: '3' },
];
...
export default function NewVisitPage() {
  ...
  const filteredMeds = COMMON_MEDICINES.filter(
    (m) => !medSearch || m.name.includes(medSearch),
  );
          ...
          <form id="visit-form">
            ...
            <Card className="border-slate-200/80 dark:border-slate-800/80">
              ...
              <CardContent className="p-4 space-y-3">
                ...
                />
                {medSearch && (
                  <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-1.5 max-h-40 overflow-y-auto">
                    <p className="text-[10px] text-slate-400 px-2 pb-1">أدوية شائعة:</p>
                    {filteredMeds.map((m) => (
                      <button
                        key={m.name}
                        type="button"
                        onClick={() => { addMedicine({ ...m }); setMedSearch(''); }}
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-400 mr-2">{m.dosage} - {m.frequency}</span>
                      </button>
                    ))}
                ...
                {medicines.map((med, i) => (
                  <div key={i} className="grid grid-cols-12 gap-1.5 items-center p-2.5 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-100 dark:border-slate-800">
                    <div className="col-span-4 space-y-0.5">
                      ...
                      <Input value={med.name} onChange={(e) => updateMedicine(i, 'name', e.target.value)} className="h-8 text-xs rounded-lg border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950" placeholder="اسم الدواء" />
                    </div>
                    <div className="col-span-2 space-y-0.5">
                      <Label className="text-[10px] text-slate-500">الجرعة</Label>
                      <Input value={med.dosage} onChange={(e) => updateMedicine(i, 'dosage', e.target.value)} className="h-8 text-xs rounded-lg border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950" placeholder="500mg" />
                    </div>
                    <div className="col-span-3 space-y-0.5">
                      <Label className="text-[10px] text-slate-500">التكرار</Label>
                      <Input value={med.frequency} onChange={(e) => updateMedicine(i, 'frequency', e.target.value)} className="h-8 text-xs rounded-lg border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950" placeholder="3 مرات يومياً" />
                    </div>
                    <div className="col-span-2 space-y-0.5">
                      <Label className="text-[10px] text-slate-500">المدة</Label>
                      <Input value={med.duration} onChange={(e) => updateMedicine(i, 'duration', e.target.value)} className="h-8 text-xs rounded-lg border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950" placeholder="أيام" />
                    </div>
